...
- **Data only.** No executable content, no URLs the app should follow, nothing about users. This applies to the rules file; the website in `docs/` is the one other thing here (see below).
- **The website in `docs/` is legal-facing.** It is the app's public Privacy Policy and Support pages, served by GitHub Pages at lottomanager.richardholland.com, and App Store Connect and the app link to those URLs. Never change the Privacy Policy's meaning without Richard's sign-off; when its wording changes, update the "Last updated" date on the page. Never move or rename `/privacy/` or `/support/`, and never delete `docs/CNAME` (that unhooks the custom domain). Terms of Use is Apple's standard EULA, linked out to and never copied here. The pages are plain HTML with no build step, so a change to the header or footer goes into every page.
